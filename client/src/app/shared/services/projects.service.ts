...
export class ProjectsService {
  ...
  public fetchGames(): Observable<{game: string, ranks: []}[]> {
    return this.http.get<{game: string, ranks: []}[]>(`./api/games`);
  }

  public fetchPlayersByRank(game: string, rank: string = ''): Observable<{firstname: string, username: string, game: string, rank: string}[]> {
    return this.http.get<{firstname: string, username: string, game: string, rank: string}[]>(`./api/players/`, {
      params: new HttpParams().set('game', game).set('rank', rank)
    });
  }
